...
{
	auto component = ompnpcng::pawn_bridge::npcs();
	if (component)
	{
...
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		ompnpcng::pawn_bridge::npcs()->destroy(npc);
		return true;
	}
	return false;
}
...
{
	int index = -1;
	auto npcs = ompnpcng::pawn_bridge::npcs();
	if (npcs)
	{
		if (outputNPCs.size() < npcs->count())
		{
			ompnpcng::pawn_bridge::core()->printLn(
...
SCRIPT_API(NPC_CreatePath, int())
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->createPath();
	}
	return false;
}
...
SCRIPT_API(NPC_DestroyPath, bool(int pathId))
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->destroyPath(pathId);
	}
	return false;
}
...
SCRIPT_API(NPC_DestroyAllPath, bool())
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		ompnpcng::pawn_bridge::npcs()->destroyAllPaths();
		return true;
	}
	return false;
}
...
SCRIPT_API(NPC_GetPathCount, int())
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->getPathCount();
	}
	return -1;
}
...
SCRIPT_API(NPC_AddPointToPath, bool(int pathId, Vector3 position, float stopRange))
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->addPointToPath(pathId, position, stopRange);
	}
	return false;
}
...
SCRIPT_API(NPC_RemovePointFromPath, bool(int pathId, int pointIndex))
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->removePointFromPath(pathId, pointIndex);
	}
	return false;
}
...
SCRIPT_API(NPC_ClearPath, bool(int pathId))
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->clearPath(pathId);
	}
	return false;
}
...
SCRIPT_API(NPC_GetPathPointCount, int(int pathId))
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->getPathPointCount(pathId);
	}
	return -1;
}
...
SCRIPT_API(NPC_GetPathPoint, bool(int pathId, int pointIndex, Vector3& position, float& stopRange))
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->getPathPoint(pathId, pointIndex, position, stopRange);
	}
	return false;
}
...
SCRIPT_API(NPC_IsValidPath, bool(int pathId))
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->isValidPath(pathId);
	}
	return false;
}
...
SCRIPT_API(NPC_HasPathPointInRange, bool(int pathId, Vector3 position, float radius))
{
	if (ompnpcng::pawn_bridge::npcs())
	{
		return ompnpcng::pawn_bridge::npcs()->hasPathPointInRange(pathId, position, radius);
	}
	return false;
}
...
{
	auto data = npc.getSurfingData();
	if (data.type != PlayerSurfingData::Type::Vehicle || ompnpcng::pawn_bridge::vehicles() == nullptr)
	{
		return INVALID_VEHICLE_ID;
	}
...
{
	auto data = npc.getSurfingData();
	if (data.type != PlayerSurfingData::Type::Object || ompnpcng::pawn_bridge::objects() == nullptr)
	{
...
SCRIPT_API_FAILRET(NPC_GetSurfingPlayerObject, INVALID_OBJECT_ID, int(INPC& npc))
{
	auto data = npc.getSurfingData();
	auto playerObjects = queryExtension<IPlayerObjectData>(npc.getPlayer());
	if (data.type != PlayerSurfingData::Type::PlayerObject || ompnpcng::pawn_bridge::objects() == nullptr || playerObjects == nullptr)
	{
		return INVALID_OBJECT_ID;
	}
